...
> 기준 화면 파일: `src/components/vc/admin/specMaster/SpecMgmt.js`  
...
| 파일 | 역할 |
| --- | --- |
| `src/components/vc/admin/specMaster/SpecMgmt.js` | 화면 container. selector로 state를 읽고 action을 dispatch한다. |
| `src/components/vc/admin/specMaster/pop/SpecMgmtPopup.js` | Master/Detail 저장 팝업. scope에 따라 필드 노출을 나눈다. |
| `src/components/vc/admin/specMaster/ui/SpecMgmtGrid.js` | Master Grid, Detail Grid, header filter, paging UI. |
| `src/components/vc/admin/specMaster/ui/SpecMgmtFields.js` | 팝업/검색에서 쓰는 select, input, switch field. |
| `src/components/vc/admin/specMaster/core/SpecMgmt.core.js` | column 정의, filter, paging, Excel helper. |
| `src/store/vc/vcMgmt/action.js` | SpecMaster action type과 action creator. |
| `src/store/vc/vcMgmt/reducer.js` | 조회, 선택, 팝업, 저장/삭제 loading state. |
| `src/store/vc/vcMgmt/vcSpecMgmtSelector.js` | 화면에서 읽는 selector. |
| `src/saga/vc/admin/vcSpecSaga.js` | API 호출 흐름과 response normalize. |
| `src/service/api/vc/admin/vcSpecApi.js` | GoodDocs 기준 HTTP API adapter. |
...
| Master radio 선택 | `GET /api/vc/specmaster/{specId}/children` | `detailRows`, 선택 Master |
...
`vcSpecSaga.js`의 `loadSpecConditionFlow`는 payload에 `selectedSpecId`가 없으면 현재 state의 `selectedSpecId`를 사용한다. 그래서 조회 버튼을 눌러도 선택 Master가 결과에 남아 있으면 그대로 유지된다.
...
Master Grid와 Detail Grid는 B/E가 내려준 전체 row를 F/E에서 `pageSize` 기준으로 자른다. 현재 기본값은 Master 10건, Detail 8건이다. B/E chunk 조회용 `/search` 흐름은 제거되었다.
