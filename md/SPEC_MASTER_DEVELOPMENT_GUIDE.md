...
`vc-app`은 로컬 preview shell용 class라 회사 화면 root에 없을 수 있다. 따라서 위 퍼블 class는 `.vc-app`에 묶지 않고 전역 class처럼 유지한다. 퍼블 CSS가 추가로 오면 기능 코드는 유지하고 위 class의 CSS만 교체하는 방향이 안전하다.
