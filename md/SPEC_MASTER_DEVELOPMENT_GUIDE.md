...
위 class는 퍼블 className 그대로 전역 class처럼 유지한다. `SpecMaster.js`는 `vc.css`를 직접 import하지 않는다. 로컬 preview에서는 `src/main.js`가 `src/vc.css`를 한 번 import하고, 회사 시스템에서는 회사 쪽 CSS가 같은 className에 적용되는 구조다.
