...
# Manual Drawing Results Grid

`BIM/5D Not Applied Fab`의 Manual Drawing Results는 B/E 전체 조회 결과를 F/E에서
4행 단위로 페이징한다. 각 데이터 컬럼의 funnel 필터는 대소문자 구분 없는 부분
일치 방식이며 전체 건수와 필터 결과 건수를 함께 표시한다.

필터 또는 재조회로 현재 선택 WO가 보이지 않으면 첫 표시 row를 자동 선택하고
Chamber 상세를 다시 조회한다. 선택 WO가 남아 있으면 해당 row가 속한 페이지를 유지한다.

Chamber 추가 기능은 별도 `Add Chamber` 버튼 대신 마지막 Chamber 탭 뒤의 작은 `+`
탭으로 표시한다. `+`를 선택하면 기존 `ADD_CHAMBER` action을 그대로 실행하며 최대
Chamber 수에 도달하면 추가 탭이 비활성화된다.
