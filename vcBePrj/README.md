...
F/E 요청에 따라 `md/SIM_API.md`와 `md/SPEC_MASTER_API.md`의 API 계약은 앞으로 조금씩 바뀔 수 있습니다.
...
Simulation API의 상세 request/response와 field 정의는 [md/SIM_API.md](../md/SIM_API.md)를 공식 계약으로 봅니다.
SpecMaster API의 상세 계약은 [md/SPEC_MASTER_API.md](../md/SPEC_MASTER_API.md)를 기준으로 봅니다.
...
| GET | `/api/vc/specmaster/{specId}/children` | SpecMaster Detail 조회 |
...
SpecMaster의 실제 화면/B/E API 계약은 [md/SPEC_MASTER_API.md](../md/SPEC_MASTER_API.md)를 기준으로 봅니다.
...
- API response field를 바꿀 때는 `md/SIM_API.md` 또는 `md/SPEC_MASTER_API.md`, response DTO, mapper, MockMvc 테스트, F/E helper를 함께 수정합니다.
...
- This API returns Master paging rows and the initially selected Master's Detail rows in one response.
...
- Master radio changes read Detail rows through `GET /api/vc/specmaster/{specId}/children`, so the front end does not re-run the Master search just to refresh the right grid.
...
- Detail lookup uses `GET /api/vc/specmaster/{specId}/children`. `POST /api/vc/specmaster/{specId}/children` is used only for Detail creation.

## F/E 미사용 / 보류 API 기록

아래 API들은 2026-06-22 기준으로 B/E에는 열려 있지만 현재 F/E 화면 어댑터와 saga에서 직접 호출하지 않습니다. 삭제 대상이라는 뜻이 아니라 화면 계약 API와 보류/legacy API를 구분하기 위한 기록입니다.

| 구분 | Method | URL | 안 쓰는 이유 / 예시 |
| --- | --- | --- | --- |
| [F/E 미사용 - 보류] | POST | `/api/vc/specmaster/selectexact` | SpecMaster 저장 전 중복 확인 UX가 아직 없습니다. 예: 같은 FAB/MODEL/공정/CHAMBER SPEC 조합을 저장 전에 막아야 하면 연결 후보입니다. |
| [F/E 미사용 - Legacy] | POST | `/api/vc/calculate` | 최신 F/E는 `/api/vc/sim/non-bim/calculate`, `/api/vc/sim/calculator/calculate`를 사용합니다. |
| [F/E 미사용 - Legacy] | GET | `/api/vc/{fabId}/requests` | 현재 F/E에는 FAB별 request header 목록 조회 화면이 없습니다. |

참고: `GET /api/vc/specs`는 완전 미사용이 아닙니다. SpecMaster 필터 옵션 API 실패 시 F/E legacy fallback으로 호출될 수 있습니다.
