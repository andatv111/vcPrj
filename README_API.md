...
이 문서는 V/C Simulation F/E가 B/E 개발팀에 요청하는 공식 API 계약입니다. B/E는 아래 endpoint, 필수값, 응답 필드와 업무 규칙을 기준으로 Controller/DTO를 구현해 주세요.
...
현재 F/E는 배열/객체 직접 응답과 `{ data }`, `{ result }`, `{ list }` wrapper를 흡수할 수 있습니다. 운영 표준은 아래 구조를 권장합니다.
...
- B/E DTO를 화면 모델로 변환하는 normalize 처리
...
| `chambers` 또는 `chamberList` | 권장 | 장비에 매핑된 Chamber 상세 |
